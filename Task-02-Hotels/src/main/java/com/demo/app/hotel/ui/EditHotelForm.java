...
public class EditHotelForm extends FormLayout {
	...
	private NativeSelect<Long> category = new NativeSelect<>("Category");
	...
	private void bindFields() {
		binder.forField(name).withValidator(name -> name.length() > 0, "Name is required").bind(Hotel::getName,
				Hotel::setName);
		binder.forField(address).withValidator(name -> name.length() > 0, "Address is required").bind(Hotel::getAddress,
				Hotel::setAddress);
		...
		binder.forField(url).withValidator(url -> url.length() > 0 && new UrlValidator().isValid(url),
				"Url is invalid. Try to write entire url").bind(Hotel::getUrl, Hotel::setUrl);
